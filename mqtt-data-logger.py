    ...
    while Log_worker_flag:
        ...
        while not q.empty():
            ...
            if options["substitute_keys"]:
                results = substitute_keys(results, options["substitute_keys"])
            if options["JSON"]:
...
def substitute_keys(output, table):
    if output['topic'] == table['topic']:
        #print("TP1", output)
        for key in table['substitude'].keys(): 
            sub_tab_entry = table['substitude'][key]
            if key in output['message']:
                msg_item = output['message'][key]
                msg_item = {(sub_tab_entry[k] if k in sub_tab_entry else k):v  for (k,v) in msg_item.items() }
                output['message'][key] = msg_item
                # print("TP2", msg_item)
    # print("TP3", output)
    return output
